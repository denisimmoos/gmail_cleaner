...
class DeleteManager:
    ...
    def delete_emails(self, message_ids: List[str], permanent: bool = False,
                      dry_run: bool = False, batch_size: int = BATCH_SIZE) -> Dict[str, int]:
        """Delete emails with progress tracking"""
        if dry_run:
            logger.info(f"🔍 DRY RUN: Would {'permanently delete' if permanent else 'trash'} {len(message_ids)} emails")
        ...
        logger.info(f"✅ Complete: {self.deleted_count} deleted, {self.failed_count} failed")
